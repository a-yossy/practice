...
use crate::dhcp::{DhcpPacket, DhcpServer};
...
fn make_dhcp_packet(
    ...
    dhcp_packet.set_option(
        ...
        4,
        Some(&dhcp_server.subnet_mast.octets()),
    );
    dhcp_packet.set_option(
        &mut cursor,
        Code::Router as u8,
...
fn dhcp_request_message_handler_responded_to_offer(
    ...
    let count = {
        ...
        match count {
            ...
            _ => database::update_query(&tx, client_macaddr, ip_to_be_leased, 0)?,
        }
...
fn dhcp_request_message_handler_to_reallocate(
    ...
    if let Some(requested_id) = received_packet.get_option(Code::RequestedIpAddress as u8) {
        debug!("clietn is in INIT_REBOOT");
        let requested_ip = util::u8_to_ipv4addr(&requested_id)
            .ok_or_else(|| failure::err_msg("Failed to convert ip addr."))?;
        let con = dhcp_server.db_connection.lock().unwrap();
        ...
        let ip_from_clinet = received_packet.get_ciaddr();
        if !dhcp_server.network_addr.contains(ip_from_clinet) {
