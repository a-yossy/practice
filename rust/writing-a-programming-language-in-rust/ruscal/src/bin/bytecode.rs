    ...
    ($($op:ident),*) => {
        impl From<u8> for OpCode {
            ...
            fn from(o: u8) -> Self {
                ...
                match o {
                    $($op => Self::$op,)*
                    _ => panic!("Opcode \"{o:02X}\" unrecognized"),
                }
            }
        }
    };
...
impl Value {
    ...
    fn coerce_f64(&self) -> f64 {
        match self {
            ...
            _ => panic!("Coercion failed: {self:?} cannot be coerced to f64"),
        }
    }
    ...
    fn coerce_i64(&self) -> i64 {
        match self {
            ...
            _ => panic!("Coercion failed: {self:?} cannot be coerced to i64"),
        }
    }
...
struct Compiler {
    ...
    funs: HashMap<String, FnByteCode>,
...
impl ByteCode {
    ...
    fn interpret_bin_op_str(
        ...
        let res = match (lhs, rhs) {
            ...
            (lhs, rhs) => panic!("Incompatible type in binary op: {lhs:?} and {rhs:?}"),
        };
        stack.push(res);
    }
...
fn tc_binary_op<'src>(
    ...
    binary_op_type(&lhst, &rhst).map_err(|_| {
        TypeCheckError::new(
            format!("Operation {op} between incompatible type: {lhst:?} and {rhst:?}",),
            lhs.span,
        )
    })
}
    ...
    Ok(match &e.expr {
        ...
        FnInvoke(str, args) => {
            ...
            let func = ctx.get_fn(**str).ok_or_else(|| {
                TypeCheckError::new(format!("function {str} is not defined"), *str)
            })?;
...
fn func_call(i: Span) -> IResult<Span, Expression> {
    ...
    Ok((r, Expression::new(ExprEnum::FnInvoke(ident, args), i)))
}
