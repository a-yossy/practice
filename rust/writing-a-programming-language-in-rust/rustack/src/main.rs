...
impl Value {
    ...
    fn to_string(&self) -> String {
        match self {
            ...
            Self::Naive(_) => "<Naive>".to_string(),
        }
    }
}

#[derive(Clone)]
struct NaiveOp(fn(&mut Vm));

impl PartialEq for NaiveOp {
...
impl std::fmt::Debug for NaiveOp {
    ...
    fn new() -> Self {
        ...
        Self {
            ...
            vars: vec![
                functions
                    .into_iter()
                    .map(|(name, fun)| (name.to_owned(), Value::Naive(NaiveOp(fun))))
                    .collect(),
            ],
            blocks: vec![],
        }
...
fn index(vm: &mut Vm) {
    ...
    let value = vm.stack[vm.stack.iter().len() - index - 1].clone();
    vm.stack.push(value);
}
